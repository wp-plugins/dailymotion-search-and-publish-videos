...
function dsp_pagination($results, $page, $size, $basequeryurl, $sort, $search){
	...
	if( $start > 1 ) {
		echo '... <a href="' . esc_url( $basequeryurl ) . '&p=1&sort='. esc_html( $sort ). '&search='.esc_html( $search ).'" class="paginator">1</a>....';
	}
	for( $i = $start; $i <= $end; $i++ ) {
		if( $i == $page ) {
		// Current page is not clickable and different from other pages
			...
			$url = add_query_arg( 'p', intval( $i ), $basequeryurl.'&sort='. esc_html( $sort ). '&search='.esc_html( $search ) );
